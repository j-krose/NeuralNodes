...
public class Genome
{
    ...
    private static int wiggleNumHiddenLayerNodes(int numHiddenLayerNodes)
    {
        ...
        return Math.max(NeuralNet.HIDDEN_LAYER_SIZE_LOWER_BOUND, Math.min(NeuralNet.HIDDEN_LAYER_SIZE_UPPER_BOUND, numHiddenLayerNodes));
    }
    ...
    public Genome()
    {
        ...
        nodesInLayer_ = new int[] { NeuralNet.NODES_IN_LAYER[0], numHiddenLayerNodes, NeuralNet.NODES_IN_LAYER[2] };
    ...
    {
        int numHiddenLayerNodes = mixGenes(genome1.nodesInLayer_[1], genome2.nodesInLayer_[1], () -> generateNumHiddenLayerNodes(), (num) -> wiggleNumHiddenLayerNodes(num));
        nodesInLayer_ = new int[] { NeuralNet.NODES_IN_LAYER[0], numHiddenLayerNodes, NeuralNet.NODES_IN_LAYER[2] };
        ...
        for (int i = 0; i < nodesInLayer_.length - 1; i++)
        {
            ...
            for (int j = 0; j < weights_[i].length; j++)
            {
                ...
                for (int k = 0; k < nodesInLayer_[i + 1]; k++)
                {
                    double weight1 = genome1.weights_[i][j % genome1.nodesInLayer_[i]][k % genome1.nodesInLayer_[i + 1]];
                    double weight2 = genome2.weights_[i][j % genome2.nodesInLayer_[i]][k % genome2.nodesInLayer_[i + 1]];
                    weights_[i][j][k] = mixGenes(weight1, weight2, () -> generateWeight(), (w) -> wiggleWeight(w));
                }
            }
        }
