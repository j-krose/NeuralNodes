    ...
    public void paintComponent(Graphics g)
    {
        ...
        for (int layer = 0; layer < layers; layer++)
        {
            ...
            for (int node = 0; node < nodesInThisLayer; node++)
            {
                ...
                graphics.fillOval(nodeCenterX - (nodeDiameter / 2), nodeCenterY - (nodeDiameter / 2), nodeDiameter, nodeDiameter);

                graphics.setColor(Color.BLACK);
                graphics.fillOval(nodeCenterX - (innerNodeDiameter / 2), nodeCenterY - (innerNodeDiameter / 2), innerNodeDiameter, innerNodeDiameter);
                ...
                if (layer == (layers - 1) && node == 0)
                {
                    ...
                    graphics.drawLine(nodeCenterX, nodeCenterY, nodeCenterX + (int) ((innerNodeDiameter / 2.0) * Math.cos(angle)),
...
//                    graphics.drawLine((int) startNodeCenter.getX(), (int) startNodeCenter.getY(), (int) endNodeCenter.getX(), (int) endNodeCenter.getY());
    ...
    private static Color nodeValueToColor(double nodeValue)
    {
        if (nodeValue > 0)
        {
            return new Color(Math.max(0.f, Math.min(1.f, 1.f - (float) nodeValue)), 1.f, Math.max(0.f, Math.min(1.f, 1.f - (float) nodeValue)));
        }
        else
        {
